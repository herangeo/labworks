...
{
    const[rows] = await pool.query("select * from GYMDATA");
    return rows;
}

async function getNote(id)
{
    const[rows] = await pool.query(`select * from GYMDATA where id=?`,[id]);
    return rows[0];
}
async function createNote(title,contents)
{
    const[result] = await pool.query(`insert into GYMDATA(title,contents) values(?,?)`,[title,contents]);
    const id=result.insertId;
    return getNote(id);
}

async function deleteNote(id) {
    const [result] = await pool.query('DELETE FROM GYMDATA WHERE id = ?', [id]);
  ...
  async function updateNote(id, newTitle, newContents) {
    const [result] = await pool.query('UPDATE GYMDATA SET title = ?, contents = ? WHERE id = ?', [newTitle, newContents, id]);
